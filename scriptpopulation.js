function createBubbleChart(error, countries, continentNames) {
  ...
  var continentColorScale = d3.scaleOrdinal(d3.schemeCategory10);
      //  .domain(continents.values()); works fine withou this
  ...
  function forceCollide(d) {
    return countryCenterGrouping() || airportsGrouping() ? 0 : circleRadiusScale(d.airports) + 1 || populationGrouping() ? 0 : circleRadiusScale(d.population) + 1|| median_ageGrouping() ? 0 : circleRadiusScale(d.median_age) + 1;
  }
  ...
  function addGroupingListeners() {
    ...
    function addListener(selector, forces) {
      d3.select(selector).on("click", function() {
        ...
        togglpopulationAxes(populationGrouping()),
